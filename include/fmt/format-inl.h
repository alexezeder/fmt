...
class fp;
template <int SHIFT = 0> constexpr fp normalize(fp value);
...
class fp {
 ...
 public:
  ...
  template <typename Double> explicit constexpr fp(Double d) { assign(d); }

  // Assigns d to this and return true iff predecessor is closer than successor.
  template <typename Float, FMT_ENABLE_IF(is_supported_float<Float>::value)>
  constexpr bool assign(Float d) {
...
template <int SHIFT> constexpr fp normalize(fp value) {
  // Handle subnormals.
...
constexpr uint64_t multiply(uint64_t lhs, uint64_t rhs) {
#if FMT_USE_INT128
...
constexpr fp operator*(fp x, fp y) {
  return {multiply(x.f, y.f), x.e + y.e + 64};
}

// Returns a cached power of 10 `c_k = c_k.f * pow(2, c_k.e)` such that its
// (binary) exponent satisfies `min_exponent <= c_k.e <= min_exponent + 28`.
constexpr fp get_cached_power(int min_exponent, int& pow10_exponent) {
  const int shift = 32;
...
class bigint {
 private:
  ...
  constexpr bigit operator[](int index) const {
    return bigits_[to_unsigned(index)];
  }
  constexpr bigit& operator[](int index) { return bigits_[to_unsigned(index)]; }
  ...
  constexpr void multiply(uint64_t value) {
 ...
 public:
  ...
  constexpr ~bigint() { FMT_ASSERT(bigits_.capacity() <= bigits_capacity, ""); }
  ...
  constexpr void assign(uint64_t n) {
    size_t num_bigits = 0;
    do {
  ...
  FMT_NOINLINE constexpr bigint& operator<<=(int shift) {
    FMT_ASSERT(shift >= 0, "");
  ...
  template <typename Int> constexpr bigint& operator*=(Int value) {
    FMT_ASSERT(value > 0, "");
    multiply(uint32_or_64_or_128_t<Int>(value));
    return *this;
  }

  friend constexpr int compare(const bigint& lhs, const bigint& rhs) {
  ...
  friend constexpr int add_compare(const bigint& lhs1, const bigint& lhs2,
                                   const bigint& rhs) {
  ...
  constexpr int divmod_assign(const bigint& divisor) {
...
constexpr round_direction get_round_direction(uint64_t divisor,
                                              uint64_t remainder,
                                              uint64_t error) {
...
FMT_ALWAYS_INLINE constexpr digits::result grisu_gen_digits(fp value,
                                                            uint64_t error,
                                                            int& exp,
                                                            Handler& handler) {
...
struct fixed_handler {
  ...
  constexpr digits::result on_start(uint64_t divisor, uint64_t remainder,
...
constexpr void fallback_format(Double d, int num_digits, bool binary32,
